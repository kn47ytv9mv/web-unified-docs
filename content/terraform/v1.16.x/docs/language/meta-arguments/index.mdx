...
You can add the experimental `enabled` argument to `resource`, `data`, and `module` blocks to turn their single instance on or off based on a condition, without the conditionally-indexed address that the `count = <condition> ? 1 : 0` pattern produces. Requires opting in to the `enabled_meta_argument` experiment. Refer to the [`enabled` reference](/terraform/language/meta-arguments/enabled) for details.
